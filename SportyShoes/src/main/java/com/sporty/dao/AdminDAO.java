...
public class AdminDAO {
	...
	public Admin authenticate(String adminId, String pwd) {
		List<Admin> list = this.sessionFactory.getCurrentSession()
				.createQuery("from Admin where admin_id=:admin_id and admin_pwd=:admin_pwd")
				.setParameter("admin_id", adminId).setParameter("admin_pwd", pwd).list();
		if (list.size() > 0)
