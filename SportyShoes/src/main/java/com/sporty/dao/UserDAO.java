...
public class UserDAO {
	...
	public User authenticate(String emailId, String pwd) {
		List<User> list = this.sessionFactory.getCurrentSession()
				.createQuery("from User where emailid=:emailid and pwd=:pwd").setParameter("emailid", emailId)
	...
	public User getUserByEmailId(String emailId) {
		List<User> list = this.sessionFactory.getCurrentSession()
				.createQuery("from User where emailid='" + emailId + "'").list();
		if (list.size() > 0)
			return (User) list.get(0);
		else
			return null;
	...
	public void updateUser(User user) {
		String sql = "";
		if (user.getID() == 0) {
			// If the user is new, set the dateAdded and save to the database
			user.setDateAdded(Calendar.getInstance().getTime());
			this.sessionFactory.getCurrentSession().save(user);
		} else if (user.getID() > 0) {
			// If the user exists, update their information
			sql = "update User set fname=:fname, lname=:lname, address=:address, age=:age, pwd=:pwd where " + " ID=:id";
	...
	// Method to retrieve a list of all users ordered by date_added
	@SuppressWarnings("unchecked")
