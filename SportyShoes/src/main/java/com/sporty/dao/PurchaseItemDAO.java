...
public class PurchaseItemDAO {
	...
	public PurchaseItem getItemById(long id) {
		...
		List<PurchaseItem> list = this.sessionFactory.getCurrentSession()
				.createQuery("from PurchaseItem where id=" + strId).list();
		if (list.size() > 0)
	...
	public List<PurchaseItem> getAllItemsByPurchaseId(long purchaseId) {
		...
		List<PurchaseItem> list = this.sessionFactory.getCurrentSession()
				.createQuery("from PurchaseItem where purchase_id=" + strId).list();
		return list;
	}
